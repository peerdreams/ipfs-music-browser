...
function addFileToIpfsS (ipfs, path) {
  return kefir.fromNodeCallback(cb => {
...
function addDirToIpfsS (ipfs, dir) {
  return kefir.fromNodeCallback(cb => {
    ipfs.util.addFromFs(dir, { recursive: true }, cb)
  }).map(last)
}
...
function addAll (ipfs, dir, cb) {
  ...
  return readdir(dir, (err, files) => {
    ...
    let ipfsDataS = kefir.combine(
      paths.map(path => addFileToIpfsS(ipfs,path))
    )
    ...
    ipfsDataS
      ...
      .flatMap(_ => addDirToIpfsS(ipfs, dir).map(x=> x.hash))
